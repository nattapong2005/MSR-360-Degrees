...
// --- 1. เปลี่ยนแปลงเงื่อนไข WHERE หลัก ---
$base_sql = "FROM answers AS ans
    ...
    WHERE subject.user_id = evaluator.user_id AND ev.period_id = " . (int)$period_id; // <--- เงื่อนไขใหม่: ผู้ประเมินและผู้ถูกประเมินคือคนเดียวกัน

// เพิ่ม filter ตามแผนกถ้าเลือกมา
$params = [];
$types = '';
if (!empty($filter_department)) {
    // กรองตามแผนกของผู้ประเมินตนเอง
...
// --- นับจำนวนและดึงข้อมูล (Logic ส่วนนี้เหมือนเดิมทั้งหมด) ---
...
$data_sql = "SELECT subject.name AS subject_name, d_subject.department_name AS subject_department, ans.score AS score, ans.comment AS comment "
    . $base_sql
...
<div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow">
    <div class="flex justify-between items-center mb-5">
        <h2 class="text-2xl font-bold text-gray-800"><i class="fa-solid fa-chart-simple"></i> รายงานผลการประเมินผลตนเอง</h2>
    ...
    <form method="GET" class="mb-6 flex items-center gap-4">
        <input type="hidden" name="page" value="self_dashboard">
        ...
        <a href="index.php?page=self_dashboard" class="text-sm text-gray-500 hover:underline ml-2">รีเซ็ต</a>
    </form>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full border-collapse border border-gray-300">
                <thead class="bg-[#16213E] text-white">
                    <tr>
                        <th class="p-2 text-left text-sm">ชื่อพนักงาน</th>
    ...
    <?php else: ?>
        <p class="text-red-600 font-medium">ไม่พบข้อมูลการประเมินตนเอง</p>
    <?php endif; ?>
</div>