...
<section class="px-4 py-6 min-h-screen">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 gap-6">
        <?php while ($row = mysqli_fetch_assoc($queryDept)) { ?>
            <a href="?page=form_question&department_id=<?= $row['department_id']; ?>" class="flex flex-col items-center justify-center text-center gap-3 w-full font-semibold bg-white p-5 rounded-lg shadow-md border border-gray-300 hover:border-red-700 group">
